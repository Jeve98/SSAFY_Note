    ...
    - 최단 경로 : 특정 정점까지의 경로 중, 가중치의 합이 최소가 되는 경로
        ex) Dijkstra, Bellman-Ford, Floyd-Warshall
            * Dijkstra : Prim과 같이 Greedy를 기반으로 시작 정점에서 특정 정점까지의 최소 누적 거리를 선택하되 음의 가중치를 허용하지 않음
                >> Greedy - 우선순위 큐에서 출력되는 순간, 최단 누적 거리가 우선해서 출력되므로 해당 정점까지의 최단 거리가 확정됨
                    >> 이로 인해, 음의 가중치가 존재할 경우, 더 짧은 경로가 발생할 수 있음에도 이를 계산하지 않게됨
            * Bellman-Ford : DP를 기반으로 시작 정점에서 특정 정점까지의 최소 누적 거리를 선택하며 음의 가중치를 허용함
                >> vertex - 1만큼 반복하며 각 정점에 대한 최단 경로를 갱신하기 때문에 sort나 우선순위 큐 등이 필요하지 않음
                    >> 단, 음의 싸이클이 존재하면 v - 1만큼의 반복 이후에도 지속적으로 최단 경로 값은 감소하므로 동작이 불가능함 (v - 1 이후에 갱신이 일어나는 것으로 음의 싸이클 확인 가능)
            * Floyd-Warshall : 모든 정점에 대해 각 정점에 대한 최단 경로를 계산
